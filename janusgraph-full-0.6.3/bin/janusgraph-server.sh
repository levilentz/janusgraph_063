...
start() {
  ...
  echo "$JANUSGRAPH_YAML will be used to start JanusGraph Server"
  ...
  if [[ -z "$RUNAS" ]]; then
    ...
    #echo $JAVA -Dlog4j.configuration=$LOG4J_CONF $JAVA_OPTIONS -cp $CLASSPATH $JANUSGRAPH_SERVER_CMD "$JANUSGRAPH_YAML" | tee cmd.sh
